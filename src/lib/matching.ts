...
// ##################################################################
// #  LAYER 1: ADVANCED NOISE CANCELLATION + QUERY EXPANSION
// ##################################################################
...
function detectIntentWithConfidence(text) {
  ...
  for (const [intentName, intentData] of Object.entries(ADVANCED_INTENT_PATTERNS)) {
    let confidence = 0;
    const matches = { strong: [], weak: [], contextual: [] };
    ...
    if (confidence > 0) {
      ...
      detectedIntents.push({
        ...
        matchType: matches.strong.length > 0 ? 'STRONG' : matches.weak.length > 0 ? 'WEAK' : 'CONTEXTUAL',
        matches: matches,
        confidenceLevel: normalizedConfidence >= 70 ? 'HIGH' : normalizedConfidence >= 40 ? 'MEDIUM' : 'LOW'
      });
    }
  }
...
function adaptiveThreshold(allScores) {
  ...
  if (gap1to2 > 30) return 65;
  if (gap1to2 > 20) return 75;
  if (gap1to2 > 10) return 85;
  return 90;
}
...
function contextualBoost(matchResult, userQuery, dbQuestion) {
  ...
  if (dbQuestion.category) {
    ...
    const catKeywords = categoryKeywords[Array.isArray(dbQuestion.category) ? dbQuestion.category[0] : dbQuestion.category] || [];
  ...
  return {
    ...
    boost,
    boostReasons,
    finalConfidence: newTotalScore >= 85 ? 'VERY_HIGH' : newTotalScore >= 70 ? 'HIGH' : newTotalScore >= 55 ? 'MEDIUM' : 'LOW'
  };
}
...
export async function intelligentQuestionMatch(userQuery, dbQuestions) {
  ...
  // STEP 1: SMART DIRECT MATCH WITH ACRONYM EXPANSION
  const expandedQuery = expandAcronyms(userQuery);
  ...
  for (const question of dbQuestions) {
    ...
    if (Math.max(directSimEn, directSimHi) >= 0.9) { // High-confidence direct match
      return {
        success: true,
        topMatch: { question, totalScore: Math.round(Math.max(directSimEn, directSimHi) * 100) },
        alternativeMatches: [],
        matchType: 'Direct',
      };
    }
