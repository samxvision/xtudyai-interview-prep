  ...
  for (const [intentName, intentData] of Object.entries(ADVANCED_INTENT_PATTERNS)) {
    ...
    if (confidence > 0) {
      ...
      const normalizedConfidence = Math.min(
        (confidence / 70) * 100, // 70 is max possible
        100
      )
...
// ==================================================================
// ABBREVIATION-AWARE DIRECT MATCH
// ==================================================================
...
function expandQueryWithEquivalents(query) {
    const words = query.toLowerCase().split(' ');
    const expandedWords = words.map(word => {
        const equivalents = ABBREVIATION_EQUIVALENTS[word] || [word];
        return `(${equivalents.join('|')})`;
    });
    return new RegExp(expandedWords.join('\\s+'), 'i');
}
...
export async function intelligentQuestionMatch(userQuery, dbQuestions) {
    ...
    // STAGE 1: Direct Match with Abbreviation Expansion
    const expandedRegex = expandQueryWithEquivalents(userQuery);
    for (const question of dbQuestions) {
        if (expandedRegex.test(question.question_en) || expandedRegex.test(question.question_hi)) {
             const simEn = stringSimilarity(userQuery, question.normalized_en);
             const simHi = stringSimilarity(userQuery, question.normalized_hi);
             if (Math.max(simEn, simHi) > 0.9) {
                return {
                    success: true,
                    topMatch: { question, totalScore: 100 },
                    alternativeMatches: [],
